...
async fn convert(
	...
	let registry_store = match registry {
		Some(reg_config) => {
			...
			// TODO: Extract servers from registry and add as MCP backends
			// This feature requires adding servers field to Registry type
			// For now, servers must be defined in the config file separately
