...
impl<R: ActionRouter> SagaExecutor<R> {
	...
	async fn execute_step_with_timeout(
		...
		match timeout {
			Some(dur) => tokio::time::timeout(dur, self.router.execute_action(action, input, Some(dur)))
				.await
	...
	fn resolve_input_binding(
		...
		match binding {
			...
			Some(InputBinding::Input { path }) => self.jsonpath_extract(saga_input, path),
			...
			Some(InputBinding::Merge(bindings)) => {
				...
				for (i, binding) in bindings.iter().enumerate() {
					let value =
						self.resolve_input_binding(&Some(binding.clone()), saga_input, step_results)?;
...
mod tests {
	...
	fn test_jsonpath_extract() {
		...
		assert_eq!(
			executor
				.jsonpath_extract(&value, "$.user.address.city")
				.unwrap(),
			serde_json::json!("NYC")
		);
