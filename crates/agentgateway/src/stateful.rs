...
impl Default for ClaimCheckInner {
    fn default() -> Self {
    ...
    fn apply_transform(&self, transform: &str, input: &Value) -> Result<Value, ExecutionError> {
        ...
        for (i, part) in parts.iter().enumerate() {
            ...
            current = current.get(*part).ok_or_else(|| {
                ExecutionError::TransformFailed(format!("field '{}' not found", part))
            })?;
        }
        Ok(current.clone())
    }
...
mod tests {
    ...
    async fn test_claim_check_no_retrieve() {
        let mock = MockToolExecutor::new()
            .with_tool_result("transform_ref", serde_json::json!({"transformed": "result"}));
        let executor = ClaimCheckExecutor::new(Arc::new(mock));
    ...
    async fn test_claim_check_with_transform() {
        let mock = MockToolExecutor::new()
            .with_tool_result("process_transformed", serde_json::json!({"result": "transformed"}));
        let executor = ClaimCheckExecutor::new(Arc::new(mock));
    ...
    async fn test_claim_check_passthrough() {
        ...
        assert!(result.is_ok(), "Expected success, got: {:?}", result);
        let value = result.unwrap();
        assert!(value.get("reference").is_some(), "Expected reference in result");
    }
}
